...
func TestWireBlindedHTRMatchesFull(t *testing.T) {
	full := &ethpb.ExecutionPayloadEnvelope{
	...
	blinded, err := blocks.WireBlindedFromFull(full)
	...
	require.NoError(t, err)
	decoded := &ethpb.WireBlindedExecutionPayloadEnvelope{}
	require.NoError(t, decoded.UnmarshalSSZ(enc))
	rtHTR, err := decoded.HashTreeRoot()
	require.NoError(t, err)
	require.Equal(t, fullHTR, rtHTR)

	signedBlinded, err := blocks.SignedWireBlindedFromFull(&ethpb.SignedExecutionPayloadEnvelope{
		Message:   full,
	...
	require.NoError(t, err)
	decodedSigned := &ethpb.SignedWireBlindedExecutionPayloadEnvelope{}
