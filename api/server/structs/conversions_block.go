	...
	return &eth.BeaconBlockFulu{
		...
		Body: &eth.BeaconBlockBodyFulu{
			...
			ExecutionPayload: &enginev1.ExecutionPayloadFulu{
				...
				BlobGasUsed:   payloadBlobGasUsed,
				ExcessBlobGas: payloadExcessBlobGas,
				ProofVersion:  payloadProofVersion,
			},
			BlsToExecutionChanges: blsChanges,
			BlobKzgCommitments:    blobKzgCommitments,
			ExecutionRequests: &enginev1.ExecutionRequests{
...
func BlindedBeaconBlockFuluFromConsensus(b *eth.BlindedBeaconBlockFulu) (*BlindedBeaconBlockFulu, error) {
	...
	payloadHeader, err := ExecutionPayloadHeaderFuluFromConsensus(b.Body.ExecutionPayloadHeader)
	if err != nil {
		return nil, err
	}
...
func ExecutionPayloadFuluFromConsensus(payload *enginev1.ExecutionPayloadFulu) (*ExecutionPayloadFulu, error) {
	baseFeePerGas, err := sszBytesToUint256String(payload.BaseFeePerGas)
	if err != nil {
		return nil, err
	}
