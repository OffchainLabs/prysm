...
# Run spectests with the Go toolchain. Each test records the spec folders it covered into
# SPEC_TEST_REPORT_OUTPUT_DIR (see testing/spectest/utils). The minimal-config suite lives in
# its own directory and needs the `minimal` build tag; mainnet + general run untagged. Spec
# test vectors are fetched on demand by build/externaldata. Extra args ("$@") pass through to
# `go test` (e.g. -run, -count=1).
export SPEC_TEST_REPORT_OUTPUT_DIR="$REPORT_DIR"
go test -tags=develop "$@" ./testing/spectest/general/... ./testing/spectest/mainnet/...
go test -tags=develop,minimal "$@" ./testing/spectest/minimal/...
