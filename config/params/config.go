...
// Spec: the ROUND_SCHEDULE MUST rules in the Round schedule config section.
func (b *BeaconChainConfig) ValidateRoundSchedule() error {
	...
	for i, e := range entries {
		...
		if e.SlotsPerRound == 0 || spe%e.SlotsPerRound != 0 {
			return errors.Errorf("round schedule: slot %d has SLOTS_PER_ROUND %d, which must be nonzero and divide SLOTS_PER_EPOCH %d", e.Slot, e.SlotsPerRound, spe)
		}
