...
func TestOrderedConfigSchedule(t *testing.T) {
	...
	for _, cfg := range params.All() {
		t.Run(cfg.ConfigName, func(t *testing.T) {
			...
			for _, fse := range params.NewOrderedSchedule(cfg) {
	...
	ofs := params.NewOrderedSchedule(bc)
	for i := range ofs {
