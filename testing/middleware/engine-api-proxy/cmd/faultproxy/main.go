...
// HTTP admin port to toggle an EL "SYNCING" fault on/off.
//
// The flags mirror ethereum-package's json_rpc_snoop (-b host, -p port,
// positional destination URL) so it is a drop-in snooper image by overriding image at snooper_params.
// The CL's --execution-endpoint then points at this proxy. With no --jwt-secret-file it
...
// All engine method versions we may need to fault.
