...
func (cfg PrysmDoppelgangerValidatorConfig) validate() error {
	if cfg.ServiceName == "" {
		return errors.New("doppelganger service name is required")
	}
	if cfg.Image == "" {
		return errors.New("doppelganger validator image is required")
	}
	if cfg.KeystoreArtifact == "" {
		return errors.New("doppelganger keystore artifact is required")
	}
	if cfg.BeaconRPC == "" {
		return errors.New("doppelganger beacon RPC provider is required")
	}
	if cfg.BeaconREST == "" {
		return errors.New("doppelganger beacon REST provider is required")
	}
	return nil
}
