...
func (v *validator) SubmitInclusionList(ctx context.Context, slot primitives.Slot, pubKey [fieldparams.BLSPubkeyLength]byte) {
	...
	slotStartTime := slots.UnsafeStartTime(v.genesisTime, slot)
	ilTxByteSize := 0
	for _, transaction := range il.Transactions {
...
func (v *validator) waitForInclusionList(ctx context.Context, slot primitives.Slot) {
	...
	if err != nil {
		tracing.AnnotateError(span, errors.Wrapf(err, "could not compute start time for slot %d", slot))
		return
	}
