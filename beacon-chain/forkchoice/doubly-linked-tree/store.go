...
func (s *Store) checkProposerBoost(block *BlockNode) {
	timeNow := time.Now()
	if timeNow.Before(s.genesisTime) {
		return
	}

	secondsIntoSlot := uint64(timeNow.Sub(s.genesisTime).Seconds()) % params.BeaconConfig().SecondsPerSlot
	currentSlot := slots.CurrentSlot(s.genesisTime)

	var boostThreshold uint64
	if slots.ToEpoch(currentSlot) >= params.BeaconConfig().EPBSForkEpoch {
		boostThreshold = params.BeaconConfig().SecondsPerSlot / params.BeaconConfig().IntervalsPerSlotEPBS
	} else {
...
func (f *ForkChoice) HighestReceivedBlockDelay() primitives.Slot {
	...
	sss, err := slots.SinceSlotStart(n.block.slot, f.store.genesisTime, n.block.timestamp)
	if err != nil {
		return 0
	}
