...
func TestSetSelfBuildExecutionPayloadBid(t *testing.T) {
	...
	src, err := vs.setExecutionPayloadBid(t.Context(), sBlk, local, nil, bidPreferences{maxPayment: 0, boostFactor: 100}, false)
	require.NoError(t, err)
...
func TestSetSelfBuildExecutionPayloadBid_BlobCommitments(t *testing.T) {
	...
	vs := &Server{}
	_, err = vs.setExecutionPayloadBid(t.Context(), sBlk, local, nil, bidPreferences{maxPayment: 0, boostFactor: 100}, true)
	require.NoError(t, err)
...
func TestSetSelfBuildExecutionPayloadBid_NilPayload(t *testing.T) {
	...
	_, err = vs.setExecutionPayloadBid(t.Context(), sBlk, nil, nil, bidPreferences{maxPayment: 0, boostFactor: 100}, false)
	require.ErrorContains(t, "local execution payload is nil", err)

	_, err = vs.setExecutionPayloadBid(t.Context(), sBlk, &consensusblocks.GetPayloadResponse{}, nil, bidPreferences{maxPayment: 0, boostFactor: 100}, false)
	require.ErrorContains(t, "local execution payload is nil", err)
}
...
func TestSetExecutionPayloadBid_SelfBuildOnlyIgnoresCache(t *testing.T) {
	...
	src, err := vs.setExecutionPayloadBid(t.Context(), sBlk, local, nil, bidPreferences{maxPayment: 0, boostFactor: 100}, true)
	require.NoError(t, err)
	require.Equal(t, bidSourceSelfBuild, src)
