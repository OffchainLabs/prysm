	...
	if err != nil {
		...
		if !selfBuildOnly {
			...
			if valErr != nil {
				...
				builderBid, builderURL = vs.getBuilderExecutionPayloadBid(ctx, head, sBlk.Block().Slot(), sBlk.Block().ParentRoot(), parentHash, pubkey, maxExecutionPayment, builderRequestAuths)
			}
		}
