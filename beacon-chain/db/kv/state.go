...
func (s *Store) saveStatesEfficientInternal(ctx context.Context, tx *bolt.Tx, blockRoots [][32]byte, states []state.ReadOnlyBeaconState, validatorKeys [][]byte, validatorsEntries map[string]*ethpb.Validator) error {
	...
	for i, rt := range blockRoots {
		...
		case *ethpb.BeaconStateDecoupled:
			if err := s.processDecoupled(ctx, rawType, rt[:], bucket, valIdxBkt, validatorKeys[i]); err != nil {
				return err
			}
		default:
...
func (s *Store) processDecoupled(ctx context.Context, pbState *ethpb.BeaconStateDecoupled, rootHash []byte, bucket, valIdxBkt *bolt.Bucket, validatorKey []byte) error {
	valEntries := pbState.Validators
	pbState.Validators = make([]*ethpb.Validator, 0)
	rawObj, err := pbState.MarshalSSZ()
	if err != nil {
	...
	encodedState := snappy.Encode(nil, append(decoupledKey, rawObj...))
	if err := bucket.Put(rootHash, encodedState); err != nil {
		return err
	}
	pbState.Validators = valEntries
...
func (s *Store) unmarshalState(_ context.Context, enc []byte, validatorEntries []*ethpb.Validator) (state.BeaconState, error) {
	...
	case hasDecoupledKey(enc):
		protoState := &ethpb.BeaconStateDecoupled{}
		if err := protoState.UnmarshalSSZ(enc[len(decoupledKey):]); err != nil {
			return nil, errors.Wrap(err, "failed to unmarshal encoding for Decoupled")
		}
		ok, err := s.isStateValidatorMigrationOver()
		if err != nil {
			return nil, err
		...
		return statenative.InitializeFromProtoUnsafeDecoupled(protoState)
...
func marshalState(ctx context.Context, st state.ReadOnlyBeaconState) ([]byte, error) {
	...
	case version.Decoupled:
		rState, ok := st.ToProtoUnsafe().(*ethpb.BeaconStateDecoupled)
		if !ok {
			return nil, errors.New("non valid inner state")
		}
		if rState == nil {
		...
		return snappy.Encode(nil, append(decoupledKey, rawObj...)), nil
	default:
