...
func RecomputeFromLayer(changedLeaves [][32]byte, changedIdx []uint64, layer [][]*[32]byte) ([32]byte, [][]*[32]byte, error) {
	if len(layer) == 0 || len(layer[0]) == 0 || layer[0][0] == nil {
		return [32]byte{}, nil, errors.New("invalid trie layer")
	}
	if changedLeaves == nil && len(changedIdx) > 0 {
		return [32]byte{}, nil, errors.New("changed leaves are nil")
	}
	if len(changedLeaves) < len(changedIdx) {
		return [32]byte{}, nil, errors.New("not enough changed leaves")
	}
...
func RecomputeFromLayerVariable(changedLeaves [][32]byte, changedIdx []uint64, layer [][]*[32]byte) ([32]byte, [][]*[32]byte, error) {
	if len(layer) == 0 || len(layer[len(layer)-1]) == 0 || layer[len(layer)-1][0] == nil {
		return [32]byte{}, nil, errors.New("invalid trie layer")
	}
	if changedLeaves == nil && len(changedIdx) > 0 {
		return [32]byte{}, nil, errors.New("changed leaves are nil")
