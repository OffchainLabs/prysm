...
// TestFieldTrie_ReleaseBase verifies that ReleaseBase correctly decrements
// the base trie's reference count, clears the overlay, and is a no-op on
// owned tries.
//
// For each supported data type (BasicArray, CompositeArray, CompressedArray):
//  1. Create trie A (owned, ref count 1).
//  2. Copy A → B (overlay, A's ref count becomes 2).
//  3. Call B.ReleaseBase(): B becomes empty, A's ref count drops to 1.
//  4. Call A.ReleaseBase(): no-op since A is owned (no base).
func TestFieldTrie_ReleaseBase(t *testing.T) {
	t.Run("BasicArray_RandaoMixes", func(t *testing.T) {
		const numMixes = 64
		length := uint64(params.BeaconConfig().EpochsPerHistoricalVector)
		...
		trieA, err := NewFieldTrie(types.RandaoMixes, types.BasicArray, mixesA, length)
		require.NoError(t, err)
		require.Equal(t, uint(1), trieA.FieldReference().Refs())

		trieB := trieA.CopyTrie()
		require.Equal(t, true, trieB.IsOverlay())
		require.Equal(t, uint(2), trieA.FieldReference().Refs())

		trieB.ReleaseBase()
		require.Equal(t, false, trieB.IsOverlay(), "B must no longer be an overlay")
		require.Equal(t, true, trieB.Empty(), "B must be empty after releasing base")
		require.Equal(t, uint(1), trieA.FieldReference().Refs(), "A ref count must be back to 1")

		// ReleaseBase on owned trie is a no-op.
		trieA.ReleaseBase()
		require.Equal(t, false, trieA.Empty(), "A must still be valid")
		require.Equal(t, uint(1), trieA.FieldReference().Refs())
	})
