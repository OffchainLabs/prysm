...
// TestLatestAncestor_FinalizedTOCTOU exercises the race between
// isFinalizedRoot and FinalizedState in latestAncestor. A concurrent
// SaveFinalizedState that interposes between the two reads would cause the
// old code to return a state belonging to a *different* root than the one
// requested. The fixed code reads root+state under a single lock and so
// either returns the correct state or falls through.
func TestLatestAncestor_FinalizedTOCTOU(t *testing.T) {
	ctx := t.Context()
	beaconDB := testDB.SetupDB(t)
	service := New(beaconDB, doublylinkedtree.New())

	const slot1, slot2 = primitives.Slot(10), primitives.Slot(20)
	s1, err := util.NewBeaconState()
	require.NoError(t, err)
	require.NoError(t, s1.SetSlot(slot1))
	s2, err := util.NewBeaconState()
	require.NoError(t, err)
	require.NoError(t, s2.SetSlot(slot2))

	r1 := [32]byte{0xaa}
	r2 := [32]byte{0xbb}

	service.SaveFinalizedState(slot1, r1, s1)
	...
	go func() {
		...
		for {
			...
			service.SaveFinalizedState(slot2, r2, s2)
			service.SaveFinalizedState(slot1, r1, s1)
		}
	}()
	...
	for range iters {
		...
		if st.Slot() != slot1 {
			close(stop)
			<-doneW
			t.Fatalf("TOCTOU: latestAncestor(r1) returned state for wrong root (slot=%d, want=%d)", st.Slot(), slot1)
		}
	}
	close(stop)
	<-doneW
