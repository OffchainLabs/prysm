...
func (s *Service) UpdateHead(ctx context.Context, proposingSlot primitives.Slot) {
	...
	// the payload timeliness policy only demotes a full head to empty, attestation weight owns promotions
	if full && !s.buildOnFull(newHeadRoot, proposingSlot, s.proposingAt(headState, proposingSlot)) {
		full = false
