...
func (f *blocksFetcher) fetchSidecars(ctx context.Context, pid peer.ID, peers []peer.ID, bwScs []blocks.BlockWithROSidecars) error {
	...
	fetchedDataColumnsByRoot, err := f.fetchMissingDataColumnsFromPeers(ctx, dataColumnBlocks, peers, batchSize)
	if err != nil {
...
func buildDataColumnByRangeRequests(roBlocks []blocks.ROBlock, missingColumnsByRoot map[[fieldparams.RootLength]byte]map[uint64]bool, batchSize int) ([]*p2ppb.DataColumnSidecarsByRangeRequest, error) {
	...
	result := make([]*p2ppb.DataColumnSidecarsByRangeRequest, 0, 1)
	...
	lastRequest := &p2ppb.DataColumnSidecarsByRangeRequest{
		StartSlot: previousStartBlockSlot,
...
func (f *blocksFetcher) fetchDataColumnsFromPeers(ctx context.Context, peers []peer.ID, targetRequest *p2ppb.DataColumnSidecarsByRangeRequest) ([]blocks.RODataColumn, error) {
	...
	dataColumnsByAdmissiblePeer, err := f.waitForPeersForDataColumns(peers, targetRequest)
	if err != nil {
		return nil, errors.Wrap(err, "wait for peers for data columns")
	}

	// Select the peers that will be requested.
	dataColumnsToFetchByPeer, err := prysmsync.SelectPeersToFetchDataColumnsFrom(targetRequest.Columns, dataColumnsByAdmissiblePeer)
	if err != nil {
	...
	for peer, columnsToFetch := range dataColumnsToFetchByPeer {
		...
		peerRoDataColumns, err := prysmsync.SendDataColumnSidecarsByRangeRequest(ctx, f.clock, f.p2p, peer, f.ctxMap, request)
		if err != nil {
...
// fetchMissingDataColumnsFromPeers looks at the blocks with given roots and retrieves all
// data columns for blocks that have commitments, and for which our store is missing data columns we should custody.
// It expects blocks to be sorted by slot.
func (f *blocksFetcher) fetchMissingDataColumnsFromPeers(ctx context.Context, blks []blocks.ROBlock, peers []peer.ID, batchSize int) (map[[fieldparams.RootLength]byte][]blocks.RODataColumn, error) {
	...
	for _, blk := range blks {
		...
		missingColumns, err := prysmsync.MissingDataColumns(blk, nodeID, actualGroupCount, f.dcs)
		if err != nil {
	...
	for len(missingColumnsByRoot) > 0 {
		...
		for _, request := range requests {
			roDataColumns, err := f.fetchDataColumnsFromPeers(ctx, peers, request)
			if err != nil {
...
func (f *blocksFetcher) waitForPeersForDataColumns(peers []peer.ID, request *p2ppb.DataColumnSidecarsByRangeRequest) (map[peer.ID]map[uint64]bool, error) {
	...
	filteredPeers, descriptions, err := f.filterPeersByTargetSlotAndBandwidth(peers, lastSlot, request.Count)
	if err != nil {
	...
	dataColumnsByAdmissiblePeer, admissiblePeersByDataColumn, moreDescriptions, err := prysmsync.AdmissiblePeersForDataColumns(filteredPeers, request.Columns, f.p2p)
	if err != nil {
