...
// unmarshalCommunicationChannelsResponse decodes an SSZ-encoded CommunicationChannelsResponse.
// Layout (SSZ container):
//
//	channels: List[CommunicationChannel, 16]
//
// CommunicationChannel layout:
//
//	protocol: List[uint8, 32]  (variable, offset at 0..4)
//	url: List[uint8, 256]      (variable, offset at 4..8)
func unmarshalCommunicationChannelsResponse(data []byte) ([]*structs.CommunicationChannel, error) {
	if len(data) < 4 {
		return nil, fmt.Errorf("SSZ communication_channels response too short: %d bytes", len(data))
	}

	listOffset := binary.LittleEndian.Uint32(data[0:4])
	if listOffset > uint32(len(data)) {
		return nil, fmt.Errorf("SSZ communication_channels response truncated")
	}

	listData := data[listOffset:]
	if len(listData) == 0 {
		return []*structs.CommunicationChannel{}, nil
	}

	// Each CommunicationChannel is variable-length.
	if len(listData) < 4 {
		return nil, fmt.Errorf("SSZ communication_channels list too short")
	}

	firstOffset := binary.LittleEndian.Uint32(listData[0:4])
	if firstOffset%4 != 0 {
		return nil, fmt.Errorf("SSZ communication_channels list first offset %d not aligned", firstOffset)
	}
	count := int(firstOffset / 4)
	if len(listData) < count*4 {
		return nil, fmt.Errorf("SSZ communication_channels list truncated")
	}
	...
	result := make([]*structs.CommunicationChannel, count)
	for i := range count {
		start := offsets[i]
		var end uint32
		if i+1 < count {
		...
		if start > uint32(len(listData)) || end > uint32(len(listData)) || start > end {
			return nil, fmt.Errorf("SSZ communication_channels item offset out of bounds")
		}
		ch, err := unmarshalCommunicationChannelItem(listData[start:end])
		if err != nil {
			return nil, errors.Wrapf(err, "unmarshal communication channel item %d", i)
		}
		result[i] = ch
	}
...
// unmarshalCommunicationChannelItem decodes a single SSZ-encoded CommunicationChannel.
// Layout:
//
//	protocol: List[uint8, 32]  (variable, offset at 0..4)
//	url: List[uint8, 256]      (variable, offset at 4..8)
func unmarshalCommunicationChannelItem(data []byte) (*structs.CommunicationChannel, error) {
	const fixedSize = 8
	if len(data) < fixedSize {
		return nil, fmt.Errorf("SSZ communication channel item too short: %d bytes", len(data))
	}

	protocolOffset := binary.LittleEndian.Uint32(data[0:4])
	urlOffset := binary.LittleEndian.Uint32(data[4:8])

	dataLen := uint32(len(data))
	if protocolOffset > dataLen || urlOffset > dataLen || protocolOffset > urlOffset {
		return nil, fmt.Errorf("SSZ communication channel item truncated")
	}

	protocol := string(data[protocolOffset:urlOffset])
	url := string(data[urlOffset:])

	return &structs.CommunicationChannel{
		Protocol: protocol,
		URL:      url,
	}, nil
}
