...
var validKinds = []kind{mainnet, minimal}
...
func run() error {
	...
	gotestsumFlags := []string{
		...
		fmt.Sprintf("--rerun-fails=%d", rerunAttempts),
	...
	failed := false
	for _, k := range kinds {
		pkgs, header, tagFlag, err := passSpec(goBin, k)
		if err != nil {
			return fmt.Errorf("pass spec for %s: %w", k, err)
		}
	...
	fmt.Printf(
		"✅ All tests passed (%s%s; any 'failure' above was a flake recovered within %d attempts)\n",
		joinKinds(kinds), suffix, rerunAttempts,
	)
...
func passSpec(goBin string, k kind) (pkgs []string, header, tagFlag string, err error) {
	...
	case mainnet:
		...
		return pkgs, "=== mainnet pass ===", "-tags=develop", err

	case minimal:
		pkgs, err = goList(goBin, strings.Fields(minimalPkgs)...)
		return pkgs, "=== minimal pass (-tags=minimal) ===", "-tags=develop,minimal", err
	}

	return nil, "", "", fmt.Errorf("unknown pass %q", k)
}

// minimalPkgs is the package set for the minimal pass (the config-dependent packages
// whose mainnet builds run in the mainnet pass).
const minimalPkgs = "./testing/spectest/minimal/... ./beacon-chain/rpc/prysm/v1alpha1/beacon " +
	"./beacon-chain/rpc/prysm/v1alpha1/validator ./config/fieldparams"

// excludeRe matches the packages dropped from the mainnet pass: E2E (heavy) and the
// minimal-config packages (which run in their own -tags=minimal pass).
var excludeRe = regexp.MustCompile(`/testing/endtoend|/testing/spectest/minimal|` +
