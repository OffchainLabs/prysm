...
func compareHeads(clients map[string]pb.BeaconChainClient) {
	...
	client1 := clients[endpt1]
	if client1 == nil {
		return
	}
	head1, err := client1.GetChainHead(context.Background(), &emptypb.Empty{})
	if err != nil {
		log.Fatal(err)
	}
	if head1 == nil {
	...
	if (head1.HeadSlot+1)%params.BeaconConfig().SlotsPerEpoch == 0 {
		p, err := client1.GetValidatorParticipation(context.Background(), &pb.GetValidatorParticipationRequest{})
		if err != nil {
			log.Fatal(err)
		}
		if p == nil {
	...
	for endpt2, client := range clients {
		...
		if !reflect.DeepEqual(head1, head2) {
			...
			if (head1.HeadSlot+1)%params.BeaconConfig().SlotsPerEpoch == 0 {
				p, err := client.GetValidatorParticipation(context.Background(), &pb.GetValidatorParticipationRequest{
