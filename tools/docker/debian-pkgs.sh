...
#
# Download, checksum-verify, and extract the exact Debian-11 userland packages that the
# Bazel image layered onto the distroless cc base, into <rootfs-dir>. This is a direct,
# byte-faithful port of tools/image_deps.bzl (same snapshot.debian.org pin + sha256, with
# the same prysmaticlabs.com/uploads mirror fallback). Together with the distroless cc base
# (glibc/libssl) these give a working bash + coreutils + /bin/sh in the final image.
#
# Package closure (Phase 5): bash + libtinfo6; coreutils + libselinux1 libpcre2-8-0 libattr1 libacl1.
set -euo pipefail
