...
func (e *ExecutionBundleFulu) UnmarshalJSON(enc []byte) error {
	// The return is actually from getPayloadV5, however since no field name is changed for json, we reuse the same struct
	dec := GetPayloadV4ResponseJson{}
	if err := json.Unmarshal(enc, &dec); err != nil {
		log.Error("execution bundle fulu dec", "error", err)
		return err
	}
	log.Debug("execution bundle fulu dec", dec)
