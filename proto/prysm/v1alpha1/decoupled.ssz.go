...
func (c *BeaconStateDecoupled) MarshalSSZTo(dst []byte) ([]byte, error) {
	...
	if len(c.AvailableCommitteeWindow) != 96 {
		return nil, ssz.ErrBytesLength
	}
...
func (c *BeaconStateDecoupled) UnmarshalSSZ(buf []byte) error {
	...
	sszSlice17 := buf[2687256:2687296] // c.JustifiedCheckpoint
	sszSlice18 := buf[2687296:2687336] // c.FinalizedCheckpoint
	sszSlice20 := buf[2687340:2711964] // c.CurrentSyncCommittee
	sszSlice21 := buf[2711964:2736588] // c.NextSyncCommittee
	sszSlice23 := buf[2736592:2736600] // c.NextWithdrawalIndex
	sszSlice24 := buf[2736600:2736608] // c.NextWithdrawalValidatorIndex
	sszSlice26 := buf[2736612:2736620] // c.DepositRequestsStartIndex
	sszSlice27 := buf[2736620:2736628] // c.DepositBalanceToConsume
	sszSlice28 := buf[2736628:2736636] // c.ExitBalanceToConsume
	sszSlice29 := buf[2736636:2736644] // c.EarliestExitEpoch
	sszSlice30 := buf[2736644:2736652] // c.ConsolidationBalanceToConsume
	sszSlice31 := buf[2736652:2736660] // c.EarliestConsolidationEpoch
	sszSlice35 := buf[2736672:2737184] // c.ProposerLookahead
	sszSlice37 := buf[2737188:2737196] // c.NextWithdrawalBuilderIndex
	sszSlice38 := buf[2737196:2738220] // c.ExecutionPayloadAvailability
	sszSlice39 := buf[2738220:2749228] // c.BuilderPendingPayments
	sszSlice41 := buf[2749232:2749264] // c.LatestBlockHash
	sszSlice43 := buf[2749268:3142484] // c.PtcWindow
	sszSlice44 := buf[3142484:3535700] // c.AvailableCommitteeWindow
	sszSlice45 := buf[3535700:3535708] // c.JustifiedHeight
	sszSlice46 := buf[3535708:3535716] // c.FinalizedHeight
	sszSlice47 := buf[3535716:3535724] // c.CurrentHeight
	sszSlice48 := buf[3535724:3535725] // c.CurrentHeightNonjustifiable
	sszSlice49 := buf[3535725:3535765] // c.CurrentHeightTarget

	sszVarOffset7 := ssz.ReadOffset(buf[524464:524468]) // c.HistoricalRoots
	if sszVarOffset7 != 3535777 {
		return ssz.ErrInvalidVariableOffset
	}
	...
	sszVarOffset15 := ssz.ReadOffset(buf[2687248:2687252]) // c.PreviousRoundParticipation
	if sszVarOffset15 > size || sszVarOffset15 < sszVarOffset12 {
		return ssz.ErrOffset
	}
	sszVarOffset16 := ssz.ReadOffset(buf[2687252:2687256]) // c.CurrentRoundParticipation
	if sszVarOffset16 > size || sszVarOffset16 < sszVarOffset15 {
		return ssz.ErrOffset
	}
	sszVarOffset19 := ssz.ReadOffset(buf[2687336:2687340]) // c.InactivityScores
	if sszVarOffset19 > size || sszVarOffset19 < sszVarOffset16 {
		return ssz.ErrOffset
	}
	sszVarOffset22 := ssz.ReadOffset(buf[2736588:2736592]) // c.LatestExecutionPayloadBid
	if sszVarOffset22 > size || sszVarOffset22 < sszVarOffset19 {
		return ssz.ErrOffset
	}
	sszVarOffset25 := ssz.ReadOffset(buf[2736608:2736612]) // c.HistoricalSummaries
	if sszVarOffset25 > size || sszVarOffset25 < sszVarOffset22 {
		return ssz.ErrOffset
	}
	sszVarOffset32 := ssz.ReadOffset(buf[2736660:2736664]) // c.PendingDeposits
	if sszVarOffset32 > size || sszVarOffset32 < sszVarOffset25 {
		return ssz.ErrOffset
	}
	sszVarOffset33 := ssz.ReadOffset(buf[2736664:2736668]) // c.PendingPartialWithdrawals
	if sszVarOffset33 > size || sszVarOffset33 < sszVarOffset32 {
		return ssz.ErrOffset
	}
	sszVarOffset34 := ssz.ReadOffset(buf[2736668:2736672]) // c.PendingConsolidations
	if sszVarOffset34 > size || sszVarOffset34 < sszVarOffset33 {
		return ssz.ErrOffset
	}
	sszVarOffset36 := ssz.ReadOffset(buf[2737184:2737188]) // c.Builders
	if sszVarOffset36 > size || sszVarOffset36 < sszVarOffset34 {
		return ssz.ErrOffset
	}
	sszVarOffset40 := ssz.ReadOffset(buf[2749228:2749232]) // c.BuilderPendingWithdrawals
	if sszVarOffset40 > size || sszVarOffset40 < sszVarOffset36 {
		return ssz.ErrOffset
	}
	sszVarOffset42 := ssz.ReadOffset(buf[2749264:2749268]) // c.PayloadExpectedWithdrawals
	if sszVarOffset42 > size || sszVarOffset42 < sszVarOffset40 {
		return ssz.ErrOffset
	}
	sszVarOffset50 := ssz.ReadOffset(buf[3535765:3535769]) // c.TargetParticipation
	if sszVarOffset50 > size || sszVarOffset50 < sszVarOffset42 {
		return ssz.ErrOffset
	}
	sszVarOffset51 := ssz.ReadOffset(buf[3535769:3535773]) // c.Progress
	if sszVarOffset51 > size || sszVarOffset51 < sszVarOffset50 {
		return ssz.ErrOffset
	}
	sszVarOffset52 := ssz.ReadOffset(buf[3535773:3535777]) // c.FinalityParticipation
	...
	// Field 39: BuilderPendingPayments
	{
		c.BuilderPendingPayments = make([]*BuilderPendingPaymentDecoupled, 64)
		for i := 0; i < 64; i++ {
	...
	// Field 43: PtcWindow
	{
		...
		for i := 0; i < 96; i++ {
			var tmp *PTCs
			tmp = new(PTCs)
			tmpSlice := sszSlice43[i*4096 : (1+i)*4096]
			if err = tmp.UnmarshalSSZ(tmpSlice); err != nil {
	...
	// Field 44: AvailableCommitteeWindow
	{
		c.AvailableCommitteeWindow = make([]*AvailableCommittee, 96)
		for i := 0; i < 96; i++ {
...
func (c *BeaconBlockBodyDecoupled) SizeSSZ() int {
	size := 404
	...
	size += len(c.AvailableAttestations) * 201
	size += len(c.SupportVotes)
	return size
}

func (c *BeaconBlockBodyDecoupled) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *BeaconBlockBodyDecoupled) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 404
	...
	// Field 13: AvailableAttestations
	dst = ssz.WriteOffset(dst, offset)
	offset += len(c.AvailableAttestations) * 201

	// Field 14: SupportVotes
	dst = ssz.WriteOffset(dst, offset)
	offset += len(c.SupportVotes)
	...
	// Field 13: AvailableAttestations
	for _, o := range c.AvailableAttestations {
		if dst, err = o.MarshalSSZTo(dst); err != nil {
			return nil, fmt.Errorf("AvailableAttestations: %w", err)
		}
	}

	// Field 14: SupportVotes
	dst = append(dst, c.SupportVotes...)
	return dst, err
}

func (c *BeaconBlockBodyDecoupled) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 404 {
		return ssz.ErrSize
	...
	sszVarOffset13 := ssz.ReadOffset(buf[396:400]) // c.AvailableAttestations
	if sszVarOffset13 > size || sszVarOffset13 < sszVarOffset12 {
		return ssz.ErrOffset
	}
	sszVarOffset14 := ssz.ReadOffset(buf[400:404]) // c.SupportVotes
	...
	sszSlice12 := buf[sszVarOffset12:sszVarOffset13] // c.ParentExecutionRequests
	sszSlice13 := buf[sszVarOffset13:sszVarOffset14] // c.AvailableAttestations
	sszSlice14 := buf[sszVarOffset14:]               // c.SupportVotes
	...
	// Field 4: AttesterSlashings
	{
		...
		if len(sszSlice4) > 3 {
			...
			c.AttesterSlashings = make([]*AttesterSlashingDecoupled, listLen)
			var tmpSlice []byte
			for i := uint64(0); i < listLen; i++ {
				var tmp *AttesterSlashingDecoupled
				tmp = new(AttesterSlashingDecoupled)
			...
			c.AttesterSlashings = make([]*AttesterSlashingDecoupled, 0)
		}
	}
	...
	// Field 5: Attestations
	{
		...
		if len(sszSlice5) > 3 {
			...
			c.Attestations = make([]*AttestationDecoupled, listLen)
			var tmpSlice []byte
			for i := uint64(0); i < listLen; i++ {
				var tmp *AttestationDecoupled
				tmp = new(AttestationDecoupled)
				endOffset := totalVarBytes
				if i+1 != listLen {
			...
			c.Attestations = make([]*AttestationDecoupled, 0)
		}
	}
	...
	// Field 13: AvailableAttestations
	{
		if len(sszSlice13)%201 != 0 {
			return fmt.Errorf("misaligned bytes: c.AvailableAttestations length is %d, which is not a multiple of 201: %w", len(sszSlice13), ssz.ErrIncorrectListSize)
		}
		numElem := len(sszSlice13) / 201
		c.AvailableAttestations = make([]*AvailableAttestation, numElem)
		for i := 0; i < numElem; i++ {
			var tmp *AvailableAttestation
			tmp = new(AvailableAttestation)
			tmpSlice := sszSlice13[i*201 : (1+i)*201]
			if err = tmp.UnmarshalSSZ(tmpSlice); err != nil {
				return fmt.Errorf("AvailableAttestations: %w", err)
			}
			c.AvailableAttestations[i] = tmp
		}
	}

	// Field 14: SupportVotes
	if err = ssz.ValidateProgressiveBitlist(sszSlice14); err != nil {
		return fmt.Errorf("SupportVotes: %w", err)
	}
	c.SupportVotes = append([]byte{}, go_bitfield.Bitlist(sszSlice14)...)
	return err
}

func (c *BeaconBlockBodyDecoupled) HashTreeRoot() ([32]byte, error) {
	return c.ProgressiveHashTreeRoot()
}

func (c *BeaconBlockBodyDecoupled) HashTreeRootWith(hh *ssz.Hasher) error {
	return c.ProgressiveHashTreeRootWith(hh)
}

var activeFieldsBeaconBlockBodyDecoupled = []byte{0b11111111, 0b01111111}

func (c *BeaconBlockBodyDecoupled) ProgressiveHashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.ProgressiveHashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *BeaconBlockBodyDecoupled) ProgressiveHashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: RandaoReveal
	if len(c.RandaoReveal) != 96 {
		return ssz.ErrBytesLength
	...
	// Field 13: AvailableAttestations
	{
		...
		hh.MerkleizeProgressiveWithMixin(subIndx, uint64(len(c.AvailableAttestations)))
	}
	// Field 14: SupportVotes
	if len(c.SupportVotes) == 0 {
		return ssz.ErrEmptyBitlist
	}
	hh.PutProgressiveBitlist(c.SupportVotes)
	hh.MerkleizeProgressiveWithActiveFields(indx, activeFieldsBeaconBlockBodyDecoupled)
	return nil
}

func (c *BeaconBlockDecoupled) SizeSSZ() int {
	size := 84
	if c.Body == nil {
		c.Body = new(BeaconBlockBodyDecoupled)
	}
	size += c.Body.SizeSSZ()
	return size
}

func (c *BeaconBlockDecoupled) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *BeaconBlockDecoupled) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 84
...
func (c *BeaconBlockDecoupled) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 84 {
		return ssz.ErrSize
...
func (c *BeaconBlockDecoupled) HashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.HashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *BeaconBlockDecoupled) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: Slot
	if err := c.Slot.HashTreeRootWith(hh); err != nil {
		return fmt.Errorf("Slot: %w", err)
...
func (c *SignedBeaconBlockDecoupled) SizeSSZ() int {
	size := 100
	if c.Block == nil {
		c.Block = new(BeaconBlockDecoupled)
	}
	size += c.Block.SizeSSZ()
	return size
}

func (c *SignedBeaconBlockDecoupled) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *SignedBeaconBlockDecoupled) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 100
...
func (c *SignedBeaconBlockDecoupled) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 100 {
		return ssz.ErrSize
...
func (c *SignedBeaconBlockDecoupled) HashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.HashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *SignedBeaconBlockDecoupled) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: Block
