...
func (c *LightClientBootstrapDataElectra) SizeSSZ() int {
	size := 328
...
func (c *LightClientBootstrapDataElectra) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *LightClientBootstrapDataElectra) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 328
...
func (c *LightClientBootstrapDataElectra) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 328 {
		return ssz.ErrSize
	...
	sszSlice1 := buf[4:196]   // c.CurrentSyncCommitteeBranch
	sszSlice3 := buf[200:328] // c.ExecutionBranch

	sszVarOffset0 := ssz.ReadOffset(buf[0:4]) // c.CurrentSyncCommittee
	if sszVarOffset0 != 328 {
		return ssz.ErrInvalidVariableOffset
	}
	if sszVarOffset0 > size {
		return ssz.ErrOffset
	}
	sszVarOffset2 := ssz.ReadOffset(buf[196:200]) // c.Execution
...
func (c *LightClientBootstrapDataElectra) HashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.HashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *LightClientBootstrapDataElectra) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: CurrentSyncCommittee
	{
		if len(c.CurrentSyncCommittee) > 1 {
...
func (c *LightClientEpochDataElectra) SizeSSZ() int {
	size := 2012
	if c.BootstrapData == nil {
		c.BootstrapData = new(LightClientBootstrapDataElectra)
	}
	size += c.BootstrapData.SizeSSZ()
	return size
}

func (c *LightClientEpochDataElectra) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *LightClientEpochDataElectra) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 2012
...
func (c *LightClientEpochDataElectra) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 2012 {
		return ssz.ErrSize
	...
	sszSlice5 := buf[1788:2012] // c.FinalityBranch

	sszVarOffset3 := ssz.ReadOffset(buf[1752:1756]) // c.BootstrapData
	if sszVarOffset3 != 2012 {
		return ssz.ErrInvalidVariableOffset
	}
	if sszVarOffset3 > size {
		return ssz.ErrOffset
...
func (c *LightClientEpochDataElectra) HashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.HashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *LightClientEpochDataElectra) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: Epoch
