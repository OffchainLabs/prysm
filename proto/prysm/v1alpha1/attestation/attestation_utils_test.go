...
func BenchmarkIsValidAttestationIndices(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if err := attestation.IsValidAttestationIndices(t.Context(), att, params.BeaconConfig().MaxValidatorsPerCommittee, params.BeaconConfig().MaxCommitteesPerSlot); err != nil {
			require.NoError(b, err)
		}
	}
}
