...
func (l *LightClientBootstrapAltair) UnmarshalSSZ(buf []byte) error {
	...
	for ii := 0; ii < 5; ii++ {
		if cap(l.CurrentSyncCommitteeBranch[ii]) == 0 {
			l.CurrentSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[1696:1856][ii*32:(ii+1)*32]))
		}
		l.CurrentSyncCommitteeBranch[ii] = append(l.CurrentSyncCommitteeBranch[ii], buf[1696:1856][ii*32:(ii+1)*32]...)
	}
...
func (l *LightClientUpdateAltair) UnmarshalSSZ(buf []byte) error {
	...
	for ii := 0; ii < 5; ii++ {
		if cap(l.NextSyncCommitteeBranch[ii]) == 0 {
			l.NextSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[1696:1856][ii*32:(ii+1)*32]))
		}
		l.NextSyncCommitteeBranch[ii] = append(l.NextSyncCommitteeBranch[ii], buf[1696:1856][ii*32:(ii+1)*32]...)
	}
	...
	if err = l.FinalizedHeader.UnmarshalSSZ(buf[1856:1968]); err != nil {
		return err
	}
	...
	for ii := 0; ii < 6; ii++ {
		if cap(l.FinalityBranch[ii]) == 0 {
			l.FinalityBranch[ii] = make([]byte, 0, len(buf[1968:2160][ii*32:(ii+1)*32]))
		}
		l.FinalityBranch[ii] = append(l.FinalityBranch[ii], buf[1968:2160][ii*32:(ii+1)*32]...)
	}
	...
	if err = l.SyncAggregate.UnmarshalSSZ(buf[2160:2260]); err != nil {
		return err
	}

	// Field (6) 'SignatureSlot'
	l.SignatureSlot = ssz.UnmarshallUint[github_com_OffchainLabs_prysm_v7_consensus_types_primitives.Slot](buf[2260:2268])
...
func (l *LightClientFinalityUpdateAltair) UnmarshalSSZ(buf []byte) error {
	...
	if err = l.SyncAggregate.UnmarshalSSZ(buf[416:516]); err != nil {
		return err
	}

	// Field (4) 'SignatureSlot'
	l.SignatureSlot = ssz.UnmarshallUint[github_com_OffchainLabs_prysm_v7_consensus_types_primitives.Slot](buf[516:524])
...
func (l *LightClientOptimisticUpdateAltair) UnmarshalSSZ(buf []byte) error {
	...
	if err = l.SyncAggregate.UnmarshalSSZ(buf[112:212]); err != nil {
		return err
	}

	// Field (2) 'SignatureSlot'
	l.SignatureSlot = ssz.UnmarshallUint[github_com_OffchainLabs_prysm_v7_consensus_types_primitives.Slot](buf[212:220])
...
func (s *SyncCommitteeContribution) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(s.AggregationBits); size != 1 {
		err = ssz.ErrBytesLengthFn("--.AggregationBits", size, 1)
		return
	}
