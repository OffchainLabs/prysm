...
// Hash: f5077d26635f16d1aa0ab2f89e0bc113440d1b822cc7384e46e9517535670734
package eth
...
// MarshalSSZ ssz marshals the CellSidecar object
func (c *CellSidecar) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(c)
}

// MarshalSSZTo ssz marshals the CellSidecar object to a target array
func (c *CellSidecar) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf

	// Field (0) 'TxHash'
	if size := len(c.TxHash); size != 32 {
	...
	// Field (1) 'Cell'
	if size := len(c.Cell); size != 2048 {
		err = ssz.ErrBytesLengthFn("--.Cell", size, 2048)
		return
	}
	dst = append(dst, c.Cell...)

	// Field (2) 'BlobIndex'
	dst = ssz.MarshalUint32(dst, c.BlobIndex)

	// Field (3) 'ColumnIndex'
	dst = ssz.MarshalUint64(dst, c.ColumnIndex)

	// Field (4) 'KzgCommitment'
	if size := len(c.KzgCommitment); size != 48 {
		err = ssz.ErrBytesLengthFn("--.KzgCommitment", size, 48)
		return
	}
	dst = append(dst, c.KzgCommitment...)

	// Field (5) 'KzgCellProof'
	if size := len(c.KzgCellProof); size != 48 {
		err = ssz.ErrBytesLengthFn("--.KzgCellProof", size, 48)
		return
	}
	dst = append(dst, c.KzgCellProof...)

	return
}

// UnmarshalSSZ ssz unmarshals the CellSidecar object
func (c *CellSidecar) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size != 2188 {
		return ssz.ErrSize
	...
	// Field (0) 'TxHash'
	if cap(c.TxHash) == 0 {
		c.TxHash = make([]byte, 0, len(buf[0:32]))
	}
	c.TxHash = append(c.TxHash, buf[0:32]...)

	// Field (1) 'Cell'
	if cap(c.Cell) == 0 {
		c.Cell = make([]byte, 0, len(buf[32:2080]))
	}
	c.Cell = append(c.Cell, buf[32:2080]...)

	// Field (2) 'BlobIndex'
	c.BlobIndex = ssz.UnmarshallUint32(buf[2080:2084])

	// Field (3) 'ColumnIndex'
	c.ColumnIndex = ssz.UnmarshallUint64(buf[2084:2092])

	// Field (4) 'KzgCommitment'
	if cap(c.KzgCommitment) == 0 {
		c.KzgCommitment = make([]byte, 0, len(buf[2092:2140]))
	}
	c.KzgCommitment = append(c.KzgCommitment, buf[2092:2140]...)

	// Field (5) 'KzgCellProof'
	if cap(c.KzgCellProof) == 0 {
		c.KzgCellProof = make([]byte, 0, len(buf[2140:2188]))
	}
	c.KzgCellProof = append(c.KzgCellProof, buf[2140:2188]...)

	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the CellSidecar object
func (c *CellSidecar) SizeSSZ() (size int) {
	size = 2188
	return
}

// HashTreeRoot ssz hashes the CellSidecar object
func (c *CellSidecar) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(c)
}

// HashTreeRootWith ssz hashes the CellSidecar object with a hasher
func (c *CellSidecar) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	...
	// Field (2) 'BlobIndex'
	hh.PutUint32(c.BlobIndex)

	// Field (3) 'ColumnIndex'
	hh.PutUint64(c.ColumnIndex)
