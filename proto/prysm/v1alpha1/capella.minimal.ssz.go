...
func (l *LightClientBootstrapCapella) UnmarshalSSZ(buf []byte) error {
	...
	for ii := 0; ii < 5; ii++ {
		if cap(l.CurrentSyncCommitteeBranch[ii]) == 0 {
			l.CurrentSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[1588:1748][ii*32:(ii+1)*32]))
		}
		l.CurrentSyncCommitteeBranch[ii] = append(l.CurrentSyncCommitteeBranch[ii], buf[1588:1748][ii*32:(ii+1)*32]...)
	}
...
func (l *LightClientUpdateCapella) UnmarshalSSZ(buf []byte) error {
	...
	for ii := 0; ii < 5; ii++ {
		if cap(l.NextSyncCommitteeBranch[ii]) == 0 {
			l.NextSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[1588:1748][ii*32:(ii+1)*32]))
		}
		l.NextSyncCommitteeBranch[ii] = append(l.NextSyncCommitteeBranch[ii], buf[1588:1748][ii*32:(ii+1)*32]...)
	}

	// Offset (3) 'FinalizedHeader'
	if o3 = ssz.ReadOffset(buf[1748:1752]); o3 > size || o0 > o3 {
		return ssz.ErrOffset
	}
	...
	for ii := 0; ii < 6; ii++ {
		if cap(l.FinalityBranch[ii]) == 0 {
			l.FinalityBranch[ii] = make([]byte, 0, len(buf[1752:1944][ii*32:(ii+1)*32]))
		}
		l.FinalityBranch[ii] = append(l.FinalityBranch[ii], buf[1752:1944][ii*32:(ii+1)*32]...)
	}
	...
	if err = l.SyncAggregate.UnmarshalSSZ(buf[1944:2044]); err != nil {
		return err
	}

	// Field (6) 'SignatureSlot'
	l.SignatureSlot = ssz.UnmarshallUint[github_com_OffchainLabs_prysm_v7_consensus_types_primitives.Slot](buf[2044:2052])
...
func (l *LightClientFinalityUpdateCapella) UnmarshalSSZ(buf []byte) error {
	...
	if err = l.SyncAggregate.UnmarshalSSZ(buf[200:300]); err != nil {
		return err
	}

	// Field (4) 'SignatureSlot'
	l.SignatureSlot = ssz.UnmarshallUint[github_com_OffchainLabs_prysm_v7_consensus_types_primitives.Slot](buf[300:308])
...
func (l *LightClientOptimisticUpdateCapella) UnmarshalSSZ(buf []byte) error {
	...
	if err = l.SyncAggregate.UnmarshalSSZ(buf[4:104]); err != nil {
		return err
	}

	// Field (2) 'SignatureSlot'
	l.SignatureSlot = ssz.UnmarshallUint[github_com_OffchainLabs_prysm_v7_consensus_types_primitives.Slot](buf[104:112])
