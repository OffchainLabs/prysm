...
func createTestProofForVerify(subnetId executionproof.ExecutionProofId, blockHash common.Hash) *executionproof.ExecutionProof {
	// We can ignore the error here as we're in a test.
	proof, _ := executionproof.NewExecutionProof(subnetId, primitives.Slot(0), blockHash, newBlockHashOrRoot(1), []byte{1, 2, 3})
	return proof
}
...
func TestDummyVerifierSuccess(t *testing.T) {
	...
	valid, err := verifier.Verifier(*proof)
	if err != nil {
...
func TestDummyVerifierMismatchedSubnet(t *testing.T) {
	...
	_, err := verifier.Verifier(*proof)
	if err == nil {
		t.Fatal("Expected verification to fail, but got no error")
	}


	if proof.ProofId != subnet1 {
		t.Errorf("Expected error for subnet %s, but got %s", subnet1, proof.ProofId)
	}
}