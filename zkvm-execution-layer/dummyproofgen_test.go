...
func TestDummyGeneratorSuccess(t *testing.T) {
	...
	result, err := generator.Generate(blockHash, blockRoot)

	if err != nil {
		t.Fatalf("Expected result to be ok, but got error: %v", err)
	}

	if result.SubnetId != subnet {
		t.Errorf("Expected subnet %s, got %s", subnet, result.SubnetId)
	}
...
func TestDummyGeneratorDeterministic(t *testing.T) {
	subnet, _ := executionproof.NewExecutionProofSubnetId(1)
	generator := NewDummyProofGenerator(subnet)
	blockHash := newBlockHashOrRoot(42)
	blockRoot := newBlockHashOrRoot(99)

	// Generate twice
	proof1, err1 := generator.Generate(blockHash, blockRoot)
	if err1 != nil {
		t.Fatalf("First generation failed: %v", err1)
	}

	proof2, err2 := generator.Generate(blockHash, blockRoot)
	if err2 != nil {
